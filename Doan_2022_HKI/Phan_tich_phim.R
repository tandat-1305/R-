...
#Hình dung các giá tr??? còn thi???u
...
fig <- plot_ly(imdb, y = ~Tong_doanh_thu,
  ...
  layout(title = 'S??? tuong quan gi???a m???c d??? th???i gian và doanh thu mang l???i',
         xaxis = list(title="M???c d??? th???i gian"),
         yaxis = list(title="T???ng doanh thu"),
         font = fontStyle)
...
fig2 <- plot_ly(Sl_theloai,
  ...
  layout(title = 'S??? lu???ng các th??? lo???i phim',
         xaxis = list(title="Th??? lo???i phim"),
         yaxis = list(title="S??? lu???ng"),
         font = fontStyle)
fig2
#---------------------------------------------------------------------------------
#Top 20 phim danh gia nhieu sao nhat, Top 20 movies with the highest rating_data
...
top_n_director <- movies_df %>%
  ...
  filter(Diem_so >= 90)
top_n_director
# Di???m IMDB VS d???o di???n
fig4 <- plot_ly(movies_df, x = ~Diem_so, y = ~Dao_dien,
                color = ~Diem_so,
                type = 'scatter', mode = 'markers',
               marker = list(opacity = 2)) %>% 
  layout(title = 'S??? tuong quan gi???a di???m s??? và d???o di???n',
         xaxis = list(title="Di???m s???"),
         yaxis = list(title="D???o di???n"),
         font = fontStyle)
fig4

#
#---------------------------------------------------------------------------------
fig <- plot_ly(movies_df, x = ~Thoi_gian, y = ~Sao, color = ~Diem_so) %>%
  layout(title = 'S??? tuong quan gi???a th???i gian, sao và di???m s???',
         xaxis = list(title="Th???i gian"),
